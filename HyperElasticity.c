...
{
  // Given F and basis values, returns B //Checked
  B[0][0] = F[0][0]*Nx;
  B[0][1] = F[1][0]*Nx;
  B[1][0] = F[0][1]*Ny;
  B[1][1] = F[1][1]*Ny;
  ...
  for (a=0; a<nen; a++) {
    ...
    for (b=0; b<nen; b++) {
      ...
      Chi[1][1]=Bb[0][1]*(Ba[0][1]*D[0][0] + Ba[1][1]*D[1][0] + Ba[2][1]*D[2][0]) +
        ...
        Na_y*(S[1][0]*Nb_x + S[1][1]*Nb_y); //Checked

      K[a][0][b][0] = G+Chi[0][0]; //Checked
      K[a][1][b][0] =   Chi[1][0];//Checked

      K[a][0][b][1] =   Chi[0][1];//Checked
      K[a][1][b][1] = G+Chi[1][1];//Checked
